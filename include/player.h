...
void descriptionItems(Items &item)
{
    ...
    case Items::goldenApple:
    {
        item.description[0] = "Este é um item cheio de poder eterno.";
        item.description[1] = "Este item lhe dará uma força incrível: " + to_string(item.damage);
        item.description[2] = "Você se sentirá revigorado: " + to_string(item.heal);
    }
...
void loopPlayer(Game &gameSaved)
{
    ...
    while (gameSaved.returnType == Game::start)
    {
        ...
        if (armadilha)
        { // se a armadilha foi ativado avisa que está no proximo movimento pós armadilha
            passado = true;
        }
